...
class GeneticAlgorithm(Generic[T]):
    def __init__(
        ...
        crossover_func: Callable[[Tuple[List[T], List[T]], int], Tuple[List[T], List[T]]],
    ...
    def run(self, max_fitness: int = 1000000, max_iteration: int = 1000, population_size: int = 10, mutation_rate: float = 0.3, num_crossover: int = 2):
        ...
        for i in range(max_iteration):
            ...
            for _ in range(int(population_size / 2 ) - 1):
                ...
                offspring_a, offspring_b = self._crossover_func(parents, num_crossover)
